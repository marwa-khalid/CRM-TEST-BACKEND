...
def create_referrer_route(referrer: ReferrerCreate,db: Session = Depends(get_session),
    tenant_id: int = Depends(get_tenant_id),actor_id: int = Depends(actor_id)):
    return ReferrerService.create_referrer(referrer, db, tenant_id,actor_id)
...
def update_referrer_route(claim_id: int, referrer: ReferrerCreate,
    db: Session = Depends(get_session), tenant_id: int = Depends(get_tenant_id),actor_id: int = Depends(actor_id)):
    return ReferrerService.update_referrer(claim_id, referrer, db, tenant_id,actor_id)
...
def search_companies_route(
    ...
    return ReferrerService.search_companies(query, db, tenant_id)
