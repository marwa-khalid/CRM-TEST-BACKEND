...
def update_vehicle_owner_service(claim_id: int, request: Request, owner_data: VehicleOwnerIn, db: Session, role: PersonRoleEnum):
    ...
    for key, new_value in owner_data.dict(
        exclude={"address", "tenant_id", "claim_id"},
        exclude_unset=True,
    ).items():
        old_value = getattr(db_owner, key)
        # Apply the value even when it's None/"" so clearing a field actually
        # persists (exclude_unset means only fields the form sent are touched).
    ...
    if owner_data.address:
        if db_owner.address:
            for key, new_value in owner_data.address.dict(exclude_unset=True).items():
